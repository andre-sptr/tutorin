...
function buildCanonicalUrl(siteUrl, slug) {
  const cleanSiteUrl = String(siteUrl ?? "").trim().replace(/\/+$/g, "");
  const cleanSlug = slugifyId(slug);
  if (!cleanSiteUrl || !cleanSlug) return null;
  return `${cleanSiteUrl}/tutorial/${cleanSlug}`;
}
...
function validateGeneratedTutorial(payload, options = {}) {
  ...
  if (imagePrompt) {
    content.push(headingBlock("AI Image Prompt", 3));
    content.push(paragraphBlock(`[Copy-paste prompt ini ke AI Image Generator]: ${imagePrompt}`));
  }
  ...
  return {
    ...
    value: {
      ...
      seo: {
        metaTitle,
        metaDescription,
        canonicalUrl: buildCanonicalUrl(options.siteUrl, slug),
      },
      content,
      imagePrompt,
    },
...
function buildTutorialDraftData(input, categoryDocumentId, tagDocumentIds) {
  const data = {
    title: input.title,
    slug: input.slug,
    ...
    tags: {
      connect: tagDocumentIds,
    },
  };

  if (input.featuredImageId !== undefined && input.featuredImageId !== null) {
    data.featuredImage = input.featuredImageId;
  } else if (input.featuredImageDocumentId) {
    data.featuredImage = input.featuredImageDocumentId;
  }
...
exports.buildTutorialDraftData = buildTutorialDraftData;
